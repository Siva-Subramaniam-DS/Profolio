...
const loadMarathonImages = () => {
  try {
    // Use Vite's recommended options: query:'?url' and import:'default'
    ...
    // Normalize values: they may be strings (URLs) or modules with default string
...
const Blog = () => {
  ...
  if (!images || images.length === 0) {
    const placeholders = Array.from({ length: 5 }).map((_, i) => generatePlaceholder(i));
    images = placeholders;
  }

  // Gallery view state - false shows featured card, true shows full gallery
  ...
  const currentAltText = useMemo(() => {
    ...
    return `Marathon photo ${galleryState.currentIndex + 1}`;
  }, [currentMetadata, galleryState.currentIndex]);
      ...
      <section id="blog" className="blog-section">
        <div className="container">
          ...
          <article className="blog-entry">
            <h3>Event — 10KM done! 🏃‍♂️</h3>
            <p>
              Chennai Runners Marathon – yesterday was all about grit, sweat, and perseverance.
              Finished 10KM in 1 hr 30 mins, starting from Napier Bridge – Marina Beach and ending at
              CPT Ground, Taramani. Grateful to share this memorable event with Vaishali Rameshbabu,
              Indian Chess Grandmaster ♟️ One city. One run. Endless memories.
            </p>
          </article>
          ...
          <div className="gallery" style={{ position: 'relative' }}>
            ...
            {images.length > 0 ? (
              <>
                {!showFullGallery ? (
                  // Featured Gallery Card (Projects-style)
                  <div className="gallery-item marathon-gallery-card">
                    <div className="gallery-image">
                      {imageLoader.getImageState(0) === 'error' ? (
                        ...
                        <ImagePlaceholder height="400px" text="Loading marathon gallery..." />
                      ) : (
                        <img 
                          src={images[0]} 
                          alt="Marathon Gallery Preview"
                          loading="eager"
                          onError={() => {
                            console.error('Featured image failed to load:', images[0]);
                          }}
                        />
                      )}
                    </div>
                    ...
                    <div className="gallery-overlay">
                      <div className="gallery-content">
                        <h3>Chennai Marathon Gallery</h3>
                        <p>
                          Relive the moments from the Chennai Runners Marathon - 10KM of grit, determination, and unforgettable memories. 
                          From the starting line at Napier Bridge to the finish at CPT Ground, Taramani.
                        </p>
                        ...
                        <div className="gallery-links">
                          <button
                            ...
                            aria-label="View full marathon gallery"
                          >
                            <span>📸</span>
                            View Gallery
                          </button>
                  ...
                  <>
                    <div style={{
                      ...
                      <h3 className="gallery-title" style={{ margin: 0 }}>
                        Marathon Gallery ({images.length} Photos)
                      </h3>
                    </div>
                    ...
                    <div 
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault();
                          galleryState.openLightbox(0);
                        }
                      }}
                      aria-label="Open featured marathon image in lightbox"
                    >
                      {imageLoader.getImageState(0) === 'error' ? (
                        <ImageErrorFallback
                          onRetry={() => imageLoader.retryImage(0)}
                        ...
                        <ImagePlaceholder height="420px" text="Loading featured image..." />
                      ) : (
                        <img 
                          src={images[0]} 
                          alt={imageMetadata.get(0) ? generateAltText(imageMetadata.get(0), 0) : "Featured Marathon photo"}
                          loading="eager"
                          onError={() => {
                            console.error('Featured image failed to load:', images[0]);
                          }}
                    ...
                    <div className="thumb-grid">
                      {images.slice(1).map((src, i) => {
                        ...
                        const altText = metadata ? generateAltText(metadata, imageIndex) : `Marathon photo ${imageIndex + 1}`;
                        ...
                        return (
                          <button 
                            key={i} 
                            className="thumb-btn" 
                            onClick={() => galleryState.openLightbox(imageIndex)} 
              ...
              <EmptyGalleryFallback message="No marathon photos found. Add images to src/assets/marathon/ to populate the gallery." />
            )}
          </div>
